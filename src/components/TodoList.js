 ...
 * @constructor
 */
function TodoList({ todos, toggleComplete, editTodo, removeTodo }) {
  return (
    <ul>
      ...
      {todos.map(todo => (
        <Todo key={todo.id} todo={todo} toggleComplete={toggleComplete} editTodo={editTodo} removeTodo={removeTodo} />
      ))}
    </ul>
  );
}
