 ...
 * @constructor
 */
function TodoList({ todos, toggleComplete, toggleEdit, removeTodo }) {
  return (
    <ul>
      ...
      {todos.map(todo => (
        <Todo key={todo.id} todo={todo} toggleComplete={toggleComplete} toggleEdit={toggleEdit} removeTodo={removeTodo} />
      ))}
    </ul>
  )
}
